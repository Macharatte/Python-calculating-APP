...
def on_click(char):
    if char == "＝":
        ...
        try:
            ...
            # 2. 単位（巨数・微小数）を数値に変換する「安全な置換」
            # 文字列の中から数字＋単位の組み合わせを探して置換
            units_map = {
            ...
            # 数字の直後に単位がある場合、その部分を (*1eX) に置き換える
            for unit, val in units_map.items():
                f = re.sub(rf'(\d+){unit}', rf'(\1*{val})', f)
...
if st.session_state.mode == "科学計算":
    ...
    st.caption("SI接頭辞 (数字の後に使用 例: 100n, 5k)")
    # 全ての単位を表示
    u_list = ["n", "μ", "m", "c", "d", "k", "M", "G", "T", "P", "Exa", "∞"]
    cols = st.columns(6)
